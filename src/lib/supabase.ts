...
import { Database, UserWebsiteSection, CollectionNew } from '@/types/database'
...
/**
 * Get user website by user_id
 * This finds the website instance for a user
 */
...
/**
 * Get website template by template_id
 */
export async function getWebsiteTemplateById(templateId: string) {
  const { data, error } = await supabase
...
/**
 * Get all template sections for a given template
 */
...
export async function getCollectionsByLabel(userId: string, collectionLabel: string): Promise<CollectionNew[]> {
  ...
  if (error) {
    console.error('Error fetching collections by label:', error)
    return []
  }
...
/**
 * Get all data needed for rendering a section
 * Returns the section config and associated collections (if applicable)
 */
export async function getSectionData(userId: string, userWebsiteId: string, sectionType: string) {
  const sections = await getUserWebsiteSections(userWebsiteId)
